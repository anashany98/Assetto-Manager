...
export default function Configuration() {
    ...
    const updateBranding = useMutation({
        mutationFn: async (data: { key: string, value: string }) => {
            await axios.post('http://localhost:8000/settings/', data);
        },
    ...
    const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            await axios.post('http://localhost:8000/settings/upload-logo', formData, {
    ...
    return (
        <div className="p-8 font-sans">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800 tracking-tight">Panel de Configuración</h1>
                    <p className="text-gray-500 mt-1">Gestión de estaciones y personalización de marca</p>
                </div>
                <div className="flex bg-gray-100 p-1.5 rounded-2xl border border-gray-200 shadow-sm">
                    <button
                        onClick={() => setActiveTab('stations')}
                        className={cn("px-6 py-2.5 rounded-xl text-sm font-black transition-all uppercase tracking-widest", activeTab === 'stations' ? "bg-white shadow-lg text-blue-600 border border-gray-100" : "text-gray-400 hover:text-gray-600")}>
                        Simuladores
                    </button>
                    <button
                        onClick={() => setActiveTab('branding')}
                        className={cn("px-6 py-2.5 rounded-xl text-sm font-black transition-all uppercase tracking-widest", activeTab === 'branding' ? "bg-white shadow-lg text-blue-600 border border-gray-100" : "text-gray-400 hover:text-gray-600")}>
                        Branding (Bar)
                    </button>
                </div>
            </div>
            ...
            {activeTab === 'stations' ? (
                <div className="grid gap-6">
                    {stations?.map((station) => (
                        <div key={station.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex items-center justify-between transition-all hover:shadow-md hover:border-blue-100">
                            <div className="flex items-center space-x-8">
                                <div className={cn(
                                    "w-16 h-16 rounded-2xl flex items-center justify-center transition-all shadow-lg",
                                    station.is_online ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"
                                )}>
                                ...
                                <div>
                                    {editingId === station.id ? (
                                        <div className="flex flex-col space-y-2">
                                            <input
                                                className="text-xl font-black text-gray-900 border-b-2 border-blue-500 focus:outline-none bg-blue-50/50 px-2 py-1 rounded"
                                                value={editForm.name}
                                        ...
                                        <div className="flex items-center space-x-3 group">
                                            <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tight">{station.name || `Simulador #${station.id}`}</h3>
                                            <button onClick={() => startEdit(station)} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-blue-500 transition-opacity">
                                                <Edit2 size={16} />
                                            </button>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex items-center space-x-6 mt-2 text-sm text-gray-500 font-bold uppercase tracking-widest">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            {editingId === station.id ? (
                                                <input
                                                    className="border-b border-gray-300 w-40 focus:outline-none bg-blue-50/50 px-2"
                                                    value={editForm.ip}
                            ...
                            <div className="flex items-center space-x-8">
                                <div className="text-right">
                                    <div className={cn(
                                        ...
                                        station.status === 'online' ? 'bg-green-100 text-green-700' :
                                            station.status === 'syncing' ? 'bg-blue-100 text-blue-700' :
                                                'bg-gray-100 text-gray-600'
                                    )}>
                                        {station.status}
                                    </div>
                                    <div className="text-[10px] text-gray-400 font-mono mt-2 uppercase tracking-widest">
                                ...
                                {editingId === station.id && (
                                    <button
                                        ...
                                        className="bg-blue-600 text-white p-3 rounded-2xl hover:bg-blue-700 transition-all shadow-lg hover:shadow-blue-500/20 active:scale-95"
                                    >
                                        <CheckCircle size={24} />
                                    </button>
                                )}
                    ...
                    {stations?.length === 0 && (
                        <div className="text-center py-24 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                            ...
                            <h3 className="text-gray-900 font-black uppercase tracking-widest text-xl">Escaneando Red...</h3>
                ...
                <div className="space-y-8 max-w-5xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {/* Bar Name Card */}
                        <div className="bg-white p-10 rounded-3xl shadow-sm border border-gray-100 hover:border-blue-100 transition-all">
                            <div className="flex items-center space-x-4 mb-8">
                                <div className="bg-blue-100 p-3 rounded-2xl text-blue-600 shadow-sm">
                                    <Layout size={24} />
                                </div>
                                <h2 className="text-2xl font-black uppercase tracking-tight">Nombre del Bar</h2>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <input
                                        className="w-full p-4 rounded-2xl border-2 border-gray-100 shadow-sm focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all text-lg font-bold"
                        ...
                        {/* Logo Card */}
                        <div className="bg-white p-10 rounded-3xl shadow-sm border border-gray-100 hover:border-purple-100 transition-all">
                            <div className="flex items-center space-x-4 mb-8">
                                <div className="bg-purple-100 p-3 rounded-2xl text-purple-600 shadow-sm">
                                    <ImageIcon size={24} />
                                </div>
                                <h2 className="text-2xl font-black uppercase tracking-tight">Logotipo</h2>
                            </div>

                            <div className="space-y-8">
                                <div className="flex flex-col items-center justify-center p-8 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200 min-h-[180px]">
                                    <img
                                        src={barLogo}
                                        alt="Preview"
                                    ...
                                    />
                                    <span className="mt-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Vista Previa</span>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex space-x-2">
                                        <input
                                            className="flex-1 p-4 rounded-2xl border-2 border-gray-100 shadow-sm focus:ring-4 focus:ring-purple-100 focus:border-purple-500 transition-all font-mono text-sm"
                                        ...
                                        <button
                                            ...
                                            className="bg-purple-600 text-white p-4 rounded-2xl hover:bg-purple-700 transition-all shadow-lg flex items-center space-x-2 shrink-0"
                                            title="Subir archivo"
                                        >
                                            <Upload size={20} />
                                            <span className="hidden sm:inline font-bold text-xs uppercase tracking-widest">Subir</span>
                                        </button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white p-10 rounded-3xl shadow-sm border border-gray-100 hover:border-yellow-100 transition-all">
                        <div className="flex items-center space-x-4 mb-8">
                            <div className="bg-yellow-100 p-3 rounded-2xl text-yellow-600 shadow-sm">
                                <Activity size={24} />
                            </div>
                            <h2 className="text-2xl font-black uppercase tracking-tight">Cinta de Noticias (Ticker)</h2>
                        </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                <label className="flex justify-between text-xs font-black text-gray-500 uppercase tracking-widest mb-3">
                                    ...
                                    <span className="text-blue-600">{branding?.find((s: any) => s.key === 'ticker_speed')?.value || '80'}s</span>
                                </label>
                                <input
                                    type="range"
                                    min="20"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {[
                                    ...
                                    <label key={item.key} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 cursor-pointer hover:bg-white hover:border-blue-200 transition-all">
                            ...
                            <div>
                                ...
                                <textarea
                                    className="w-full p-4 rounded-2xl border-2 border-gray-100 shadow-sm focus:ring-4 focus:ring-yellow-100 focus:border-yellow-500 transition-all text-lg font-bold min-h-[100px]"
                            ...
                            <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 flex items-center justify-between">
                                <div>
                                    <label className="block text-gray-900 font-black text-sm uppercase tracking-widest mb-1">Narrador de Voz (TTS)</label>
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        (branding?.find((s: any) => s.key === 'enable_tts')?.value || 'true') === 'true' ? "bg-green-500" : "bg-gray-300"
                                    )}
                                >
                                    <div className={cn(
                            ...
                            <div className="bg-yellow-50/50 p-4 rounded-2xl border border-yellow-100">
                                <p className="text-xs text-yellow-700 font-bold italic leading-relaxed">
                    ...
                    <div className="bg-white p-10 rounded-3xl shadow-sm border border-gray-100 hover:border-blue-100 transition-all">
                        <div className="flex items-center space-x-4 mb-8">
                            ...
                            <h2 className="text-2xl font-black uppercase tracking-tight">QR Code Leaderboard</h2>
                        </div>

                        <div className="space-y-6">
                            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 cursor-pointer hover:bg-white hover:border-blue-200 transition-all">
                                <div>
                                    <span className="text-sm font-bold text-gray-700 block">Mostrar Código QR</span>
                            ...
                            <div>
                                ...
                                <input
                                    className="w-full p-4 rounded-2xl border-2 border-gray-100 shadow-sm focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all font-mono text-sm"
                                ...
                                <div className="flex justify-between items-center mt-3">
                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                                        Tu IP actual detectada: <span className="text-blue-500">{window.location.hostname}</span>
                                    </p>
                                    <button
                                        onClick={() => {
                                        ...
                                        className="text-[9px] bg-blue-50 hover:bg-blue-100 text-blue-600 px-2 py-1 rounded font-black uppercase tracking-tighter transition-colors"
                                    >
                                        Usar esta IP
                                    </button>
                                </div>
                                ...
                                <div className="mt-6 pt-6 border-t border-gray-100 grid grid-cols-2 gap-3">
                                    <a
                                        href="/tv/leaderboard"
                                        target="_blank"
                                        className="flex items-center justify-center space-x-2 p-3 bg-gray-50 hover:bg-white hover:border-blue-500 border-2 border-transparent rounded-xl transition-all group shadow-sm"
                                    >
                                        <div className="p-2 bg-blue-100 rounded-lg text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                                            <Activity size={16} />
                                        </div>
                                        <div className="text-left">
                                            <span className="block text-[10px] font-black uppercase text-gray-400">Ver Dashboard</span>
                                            <span className="block text-xs font-bold text-gray-700">Pantalla TV</span>
                                        </div>
                                    </a>
                                    <a
                                        href="/mobile"
                                        target="_blank"
                                        className="flex items-center justify-center space-x-2 p-3 bg-gray-50 hover:bg-white hover:border-green-500 border-2 border-transparent rounded-xl transition-all group shadow-sm"
                                    >
                                        ...
                                        <div className="text-left">
                                            <span className="block text-[10px] font-black uppercase text-gray-400">Ver App Móvil</span>
                                            <span className="block text-xs font-bold text-gray-700">Vista Cliente</span>
                                        </div>
                                    </a>
                                </div>
                            </div>
