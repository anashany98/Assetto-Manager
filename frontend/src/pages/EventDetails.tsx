...
export default function EventDetails() {
    ...
    if (!event) return (
        <div className="p-10 text-center text-[var(--text-primary)] flex flex-col items-center justify-center min-h-[400px]">
            <Trophy size={48} className="text-[var(--text-tertiary)] mb-4 opacity-20" />
            <p className="font-bold text-[var(--text-tertiary)] uppercase tracking-widest text-sm">Evento no encontrado</p>
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto min-h-screen bg-gray-50">
            {/* Header */}
            <Link to="/events" className="inline-flex items-center text-[var(--text-tertiary)] hover:text-blue-600 mb-6 transition-colors">
                <ArrowLeft size={20} className="mr-2" />
                Volver a Torneos
            </Link>

            <div className="bg-[var(--bg-card)] rounded-2xl shadow-xl overflow-hidden mb-8 border border-gray-100">
                <div className={cn(
                    "h-4 w-full",
                ...
                <div className="p-8 relative overflow-hidden">
                    <div className="relative z-10 flex justify-between items-start">
                        <div>
                            ...
                            <p className="text-[var(--text-tertiary)] text-lg mb-6">{event.description}</p>
                            ...
                            <div className="flex flex-wrap gap-6 text-gray-600 items-center">
                                ...
                                <div className="flex items-center bg-gray-100 px-4 py-2 rounded-lg">
                                    <Clock className="mr-2 text-[var(--text-tertiary)]" size={20} />
                                ...
                                <Link
                                    ...
                                    className="flex items-center bg-[var(--bg-card)] text-[var(--text-primary)] px-4 py-2 rounded-lg hover:bg-black transition-colors shadow-lg ml-2"
                                >
            ...
            <div className="flex space-x-6 mb-8 border-b border-gray-200">
                <button
                    ...
                    className={`pb-4 px-2 font-bold text-lg border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'leaderboard' ? 'border-blue-600 text-blue-600' : 'border-transparent text-[var(--text-tertiary)] hover:text-gray-600'}`}
                >
                    <Trophy size={20} /> Clasificación
                </button>
                <button
                    onClick={() => setActiveTab('bracket')}
                    className={`pb-4 px-2 font-bold text-lg border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'bracket' ? 'border-blue-600 text-blue-600' : 'border-transparent text-[var(--text-tertiary)] hover:text-gray-600'}`}
                >
                    <Users size={20} /> Eliminatorias
                </button>
                <button
                    onClick={() => setActiveTab('admin')}
                    className={`pb-4 px-2 font-bold text-lg border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'admin' ? 'border-red-600 text-red-600' : 'border-transparent text-[var(--text-tertiary)] hover:text-gray-600'}`}
                >
            ...
            {/* Content */}
            <div className="bg-[var(--bg-card)] rounded-2xl shadow-xl border border-gray-100 min-h-[500px]">
                {activeTab === 'leaderboard' ? (
                    ...
                    <div className="p-4 bg-[var(--bg-card)] rounded-b-xl h-full min-h-[500px]">
                        <TournamentBracket eventId={eventId} isAdmin={true} />
                    </div>
                ) : (

                    <div className="p-8 bg-[var(--bg-card)] rounded-b-xl h-full min-h-[500px]">
