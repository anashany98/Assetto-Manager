...
export default function TournamentBracket({ eventId, isAdmin = false }: { eventId: number, isAdmin?: boolean }) {
    ...
    if (isLoading) return (
        <div className="p-8 text-center text-[var(--text-primary)] flex flex-col items-center justify-center min-h-[200px]">
    ...
    return (
        <div className="w-full h-full overflow-x-auto p-8">
            {(!bracket || rounds.length === 0) ? (
                isAdmin ? (
                    <div className="text-center py-12 bg-[var(--bg-card)]/5 rounded-xl border border-white/10">
                        <Trophy size={48} className="mx-auto text-yellow-500 mb-4" />
                        <h3 className="text-xl font-bold text-[var(--text-primary)] mb-4">Generar Cuadro del Torneo</h3>
                        <p className="text-[var(--text-tertiary)] mb-6">Selecciona el tamaño del cuadro basado en la clasificación actual.</p>
                        <div className="flex justify-center gap-4">
                            {[4, 8, 16].map(size => (
                                <button
                                    ...
                                    className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-[var(--text-primary)] rounded-lg font-bold transition-colors"
                                >
                                    Top {size}
                                </button>
                            ))}
                    ...
                    <div className="text-center text-[var(--text-tertiary)] py-12">El cuadro del torneo aún no ha sido generado.</div>
                )
            ) : (
                        ...
                        <div key={roundIndex} className="flex flex-col gap-8 relative pt-12">
                            ...
                            {Array.isArray(round) && [...round].sort((a, b) => (a.match_num || 0) - (b.match_num || 0)).map((match: Match) => (
                                <div key={match.id} className="relative w-64">
                                    <div className={`bg-[#1a1c23] border ${match.winner ? 'border-yellow-500/50' : 'border-[var(--border-default)]'} rounded-lg overflow-hidden shadow-xl`}>
                                        <div className="flex justify-between items-center text-xs text-[var(--text-tertiary)] px-3 py-1 bg-black/20 border-b border-white/5">
                                            <span>Match #{match.match_num}</span>
                                            {match.winner && <span className="text-green-500">Completado</span>}
                                        </div>

                                        <div
                                            className={`px-4 py-3 border-b border-white/5 flex justify-between items-center cursor-pointer hover:bg-[var(--bg-card)]/5 transition-colors ${match.winner === match.player1 ? 'bg-yellow-500/10' : ''}`}
                                            onClick={() => isAdmin && match.player1 && setWinnerMutation.mutate({ matchId: match.id, winner: match.player1 })}
                                        >
                                            <span className={`font-bold ${match.winner === match.player1 ? 'text-yellow-400' : 'text-[var(--text-secondary)]'} ${!match.player1 ? 'italic text-gray-600' : ''}`}>
                                                {match.player1 || 'BYE'}
                                            </span>
                                        ...
                                        <div
                                            className={`px-4 py-3 flex justify-between items-center cursor-pointer hover:bg-[var(--bg-card)]/5 transition-colors ${match.winner === match.player2 ? 'bg-yellow-500/10' : ''}`}
                                            onClick={() => isAdmin && match.player2 && setWinnerMutation.mutate({ matchId: match.id, winner: match.player2 })}
                                        >
                                            <span className={`font-bold ${match.winner === match.player2 ? 'text-yellow-400' : 'text-[var(--text-secondary)]'} ${!match.player2 ? 'italic text-gray-600' : ''}`}>
                                                {match.player2 || 'BYE'}
                                            </span>
            ...
            {Array.isArray(rounds) && rounds.length > 0 && Array.isArray(rounds[rounds.length - 1]) && rounds[rounds.length - 1][0]?.winner && (
                <div className="flex flex-col items-center justify-center mt-20 animate-in zoom-in duration-500">
                    ...
                    <div className="text-3xl font-black text-[var(--text-primary)] bg-gradient-to-r from-yellow-600 to-yellow-400 bg-clip-text text-transparent">
