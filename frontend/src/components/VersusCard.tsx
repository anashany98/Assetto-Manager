...
export function VersusCard({ drivers, track }: Props) {
    ...
    if (isLoading) return <div className="text-center text-4xl text-[var(--text-primary)] animate-pulse font-black italic mt-40">ANALIZANDO RIVALES...</div>;
    ...
    return (
        <div className="h-full w-full bg-[var(--bg-app)] text-[var(--text-primary)] flex flex-col p-4 md:p-8 overflow-hidden relative">
            {/* Background Texture */}
            ...
            <header className="text-center mb-8 relative z-10 shrink-0">
                ...
                <p className="text-2xl text-[var(--text-tertiary)] mt-2 font-mono tracking-widest uppercase">{track}</p>
            </header>
                    ...
                    return (
                        <motion.div
                            ...
                            className={cn(
                                ...
                                getColumnColor(idx),
                                isP1 ? "border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.3)] scale-[1.02] z-20" : "border-[var(--border-default)]"
                            )}
                        >
                            {/* Rank Badge */}
                            <div className={cn(
                            ...
                            <div className="flex-1 p-6 flex flex-col justify-center space-y-6">
                                {/* Best Lap */}
                                <div>
                                    <p className="text-xs uppercase tracking-widest text-[var(--text-tertiary)] font-bold mb-1">Mejor Vuelta</p>
                                    <p className={cn(
                                        "text-4xl md:text-5xl font-mono font-bold tracking-tighter",
                                        isP1 ? "text-yellow-400" : "text-[var(--text-primary)]"
                                    )}>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-black/30 p-3 rounded-xl border border-white/5">
                                        <p className="text-[10px] uppercase tracking-wider text-[var(--text-tertiary)] mb-1">Consistencia</p>
                                        <p className="text-2xl font-bold text-[var(--text-secondary)]">
                                            {driver.consistency}ms
                                        </p>
                                    </div>
                                    <div className="bg-black/30 p-3 rounded-xl border border-white/5">
                                        <p className="text-[10px] uppercase tracking-wider text-[var(--text-tertiary)] mb-1">Vueltas</p>
                                        <p className="text-2xl font-bold text-[var(--text-secondary)]">
                                            {driver.total_laps}
                                        </p>
                                    </div>
                            ...
                            {winCount > 0 && (
                                <div className="bg-[var(--bg-card)]/5 p-2 flex justify-center space-x-1">
