...
export function TournamentLeaderboard({ eventId, eventName, description }: Props) {
    ...
    if (isLoading) return (
        <div className="flex flex-col items-center justify-center p-20 text-[var(--text-primary)] min-h-[400px]">
    ...
    return (
        <div className="h-full bg-[var(--bg-card)] text-[var(--text-primary)] p-8">
            <header className="mb-8 text-center border-b border-[var(--border-default)] pb-6">
                ...
                <p className="text-xl text-[var(--text-tertiary)]">{description || "Clasificación en tiempo real"}</p>
            </header>
            ...
            {safeLeaderboard.length === 0 ? (
                ...
                <div className="overflow-hidden rounded-2xl border border-[var(--border-default)] bg-[var(--bg-elevated)]/20 backdrop-blur-sm">
                    <table className="w-full text-left">
                        <thead className="bg-[var(--bg-elevated)]/50 text-[var(--text-tertiary)] uppercase tracking-widest text-sm font-bold">
                            <tr>
                            ...
                            {safeLeaderboard.slice(0, 10).map((entry, index) => (
                                <tr key={entry.lap_id} className={cn(
                                    ...
                                    index === 0 ? "bg-yellow-500/10 text-yellow-400" :
                                        index === 1 ? "bg-gray-500/10 text-[var(--text-secondary)]" :
                                            index === 2 ? "bg-orange-500/10 text-orange-400" : "text-[var(--text-primary)] hover:bg-[var(--bg-card)]/5"
                                )}>
                                    <td className="p-6 pl-8">
                                        <div className={cn(
                                            "w-12 h-12 flex items-center justify-center rounded-lg border-2",
                                            index === 0 ? "border-yellow-500 bg-yellow-500/20" :
                                                "border-[var(--border-default)] bg-gray-800"
                                        )}>
                                            {index + 1}
                                        </div>
                                    </td>
                                    ...
                                    <td className="p-6 text-lg text-[var(--text-tertiary)] font-medium">{entry.car_model}</td>
