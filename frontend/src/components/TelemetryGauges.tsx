        ...
        <div className="h-full w-full bg-gradient-to-br from-gray-900 via-black to-gray-900 p-8 grid grid-cols-12 gap-6">
            ...
            <div className="col-span-3 flex flex-col justify-center space-y-6">
                <div className="bg-gradient-to-r from-yellow-500/20 to-transparent border-l-4 border-yellow-500 p-6 rounded-r-2xl">
                    ...
                    <h2 className="text-4xl font-black text-[var(--text-primary)] uppercase tracking-tighter leading-none">{data.driver || 'Desconocido'}</h2>
                </div>
                <div className="bg-[var(--bg-card)]/5 border border-white/10 p-4 rounded-xl space-y-2">
            ...
            <div className="col-span-6 flex flex-col items-center justify-center">
                ...
                <div className="relative w-72 h-72 mb-8">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-7xl font-black text-[var(--text-primary)] tabular-nums">{Math.round(data.speed_kmh)}</span>
                        <span className="text-sm font-bold text-[var(--text-tertiary)] uppercase tracking-widest">KM/H</span>
                    </div>
                </div>
                ...
                <div className="mt-6 text-center">
                    <span className="text-8xl font-black text-[var(--text-primary)]">{data.gear === 0 ? 'N' : data.gear === -1 ? 'R' : data.gear}</span>
                    <p className="text-xs text-[var(--text-tertiary)] uppercase tracking-widest mt-1">Marcha</p>
                </div>
            </div>
            ...
            <div className="col-span-3 flex flex-col justify-center space-y-6">
                {/* Pedals */}
                <div className="bg-[var(--bg-card)]/5 border border-white/10 p-4 rounded-xl">
                    <h3 className="text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-widest mb-4">Pedales</h3>
                ...
                {/* Steering */}
                <div className="bg-[var(--bg-card)]/5 border border-white/10 p-4 rounded-xl">
                    <h3 className="text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-widest mb-2">Volante</h3>
                    <div className="h-2 bg-[var(--bg-elevated)] rounded-full relative overflow-hidden">
                        <motion.div
                ...
                {/* G-Forces */}
                <div className="bg-[var(--bg-card)]/5 border border-white/10 p-4 rounded-xl">
                    <h3 className="text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-widest mb-2">Fuerzas G</h3>
                    <div className="relative w-24 h-24 mx-auto bg-[var(--bg-elevated)] rounded-full">
                        <motion.div
...
function InfoRow({ label, value }: { label: string; value: string }) {
    return (
        <div className="flex justify-between">
            ...
            <span className="text-[var(--text-primary)] font-bold text-sm truncate max-w-[150px]">{value}</span>
        </div>
    );
}
...
function PedalBar({ label, value, color }: { label: string; value: number; color: 'green' | 'red' }) {
    ...
    return (
        <div className="flex-1 text-center">
            <div className="h-32 bg-[var(--bg-elevated)] rounded-lg relative overflow-hidden">
                <motion.div
            ...
            <p className="text-xs text-[var(--text-tertiary)] mt-2 uppercase">{label}</p>
            <p className="text-sm font-bold text-[var(--text-primary)]">{Math.round(value * 100)}%</p>
        </div>
    );
}
...
function TempCard({ label, value, unit, warning }: { label: string; value: number; unit: string; warning?: boolean }) {
    return (
        <div className={`p-3 rounded-xl text-center ${warning ? 'bg-red-500/20 border border-red-500/30' : 'bg-[var(--bg-card)]/5 border border-white/10'}`}>
            <p className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-widest">{label}</p>
            <p className={`text-xl font-bold ${warning ? 'text-red-400 animate-pulse' : 'text-[var(--text-primary)]'}`}>
