...
export function LicenseSettings() {
    ...
    return (
        <div className="space-y-6 max-w-4xl">
            <h2 className="text-xl font-bold text-white mb-4">Estado de Licencia</h2>
            ...
            <div className={`p-6 rounded-xl border ${license?.is_valid ? 'bg-green-500/10 border-green-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <div className="text-2xl font-bold text-white">{license?.client || "Sin Licencia"}</div>
                    </div>
                    <div className={`px-3 py-1 rounded-full text-sm font-bold ${license?.is_valid ? 'bg-green-500 text-black' : 'bg-red-500 text-white'}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <div className="text-white font-medium">{license?.valid_until ? new Date(license.valid_until).toLocaleDateString() : "-"}</div>
                    </div>
                    <div>
                        <div className="text-sm text-slate-500">Días Restantes</div>
                        <div className={`font-medium ${license?.days_remaining && license?.days_remaining < 30 ? 'text-orange-400' : 'text-white'}`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {license?.modules.includes('*') ? (
                            ...
                            license?.modules.map(m => (
                                <span key={m} className="px-2 py-1 rounded bg-slate-800 text-slate-300 text-xs border border-slate-700">{m}</span>
                            ))
                        )}
            ...
            <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700">
                <h3 className="text-lg font-medium text-white mb-4">Activar Nueva Licencia</h3>
                <textarea
                    value={key}
                    onChange={(e) => setKey(e.target.value)}
                    placeholder="Pegue su clave de licencia aquí (inicia con eyJ...)"
                    className="w-full h-32 bg-slate-900 border border-slate-700 rounded-lg p-4 text-slate-300 font-mono text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent mb-4"
                />
                <button
